...
class NoiseAdder:
    ...
    def apply_noise(self, image, noise_type: str, strength):
        if noise_type.lower() not in self.noise_types:
            raise ValueError(
                f"Invalid noise type. Allowed types: {', '.join(self.noise_types)}"
            )
    ...
    def map_value(
        self, value, lower_range, upper_range, lower_range_new, upper_range_new
    ):

        mapped_value = (
            (value - lower_range)
            * (upper_range_new - lower_range_new)
...
class ImageFilter:
    ...
    def apply_filter(self, image, filter_type: str, filter_size, **kwargs):
        if filter_type.lower() not in self.filter_types:
            raise ValueError(
                f"Invalid filter type. Allowed types: {', '.join(self.filter_types)}"
            )
    ...
    def average(self, image, filter_size, **kwargs):
        ...
        if image.colored:
            ...
            for ch in range(num_channels):
                filtered_image[:, :, ch] = self.convolve_2d(
                    image.manipulated_img[:, :, ch], kernel, False
                )
        else:
    ...
    def fuzzy(self, image, filter_size, **kwargs):
        ...
        if image.colored:
            ...
            for ch in range(num_channels):
                filtered_image[:, :, ch] = self.convolve_2d(
                    image.manipulated_img[:, :, ch], kernel, True
                )
        else:
    ...
    def median(self, image, filter_size, **kwargs):
        if image.colored:
            ...
            filtered_image = np.zeros_like(image.manipulated_img)
            for ch in range(num_channels):
                filtered_image[:, :, ch] = self.median_2d(
                    image.manipulated_img[:, :, ch], filter_size
                )
            return filtered_image
        else:
    ...
    def compute_bilateral_2d(self, image, filter_size, sigma_space, sigma_color):
        ...
        color_kernel = lambda difference: np.exp(
            -(difference**2) / (2 * sigma_color**2)
        ) / (np.sqrt(2 * np.pi) * sigma_color)

        filtered_image = np.zeros_like(image)
        for x in range(width):
            for y in range(height):
                neighbourhood = padded_image[y : y + filter_size, x : x + filter_size]
                color_difference = abs(
                    neighbourhood - padded_image[y + pad_size, x + pad_size]
                )
                brightness_kernel = color_kernel(
                    color_difference
                )  # this kernal changes according to the brightness difference, which makes this filter non-linear

                weighted_sum = np.sum(
                    neighbourhood * spatial_kernel * brightness_kernel
                )
    ...
    def bilateral(self, image, filter_size, **kwargs):
        ...
        if image.colored:
            ...
            filtered_image = np.zeros_like(image.manipulated_img)
            for ch in range(num_channels):
                filtered_image[:, :, ch] = self.compute_bilateral_2d(
                    image.manipulated_img[:, :, ch],
                    filter_size,
                    sigma_space,
                    sigma_color,
                )
        else:
            filtered_image = self.compute_bilateral_2d(
                image.manipulated_img, filter_size, sigma_space, sigma_color
            )
...
class EdgeDetector:
    ...
    def apply_detector(self, image, edge_detector, filter_flag):
        if edge_detector.lower() not in self.edge_detectors:
            raise ValueError(
                f"Invalid filter type. Allowed types: {', '.join(self.edge_detectors)}"
            )
        if filter_flag:
...
class Image:
    ...
    def convert_to_grayscale(self):
        if self.colored:
            ...
            self.manipulated_img = np.dot(
                self.manipulated_img[..., :3], [0.2989, 0.5870, 0.1140]
            )
            self.colored = False
    ...
    def plot_intensity_histogram(self):
        ...
        if not self.colored:
            plt.hist(
                self.manipulated_img.flatten(), bins=256, color="gray", density=True
            )
            ...
            for i, color in enumerate(["red", "green", "blue"]):
                plt.hist(
                    self.manipulated_img[:, :, i].flatten(),
                    bins=256,
                    color=color,
                    alpha=0.5,
    ...
    def adjust_brightness(self, delta):
        self.manipulated_img = np.clip(self.manipulated_img + delta, 0, 255).astype(
            np.uint8
        )
...
class ImageProcessor:
    ...
    def add_noise(
        self, image, noise_type: str = "uniform", strength=1
    ):  # strength represent a slider (0 --> 100)
        noisy_image = self.noise_source.apply_noise(
            image.manipulated_img, noise_type, strength
        )
        image.previous_img = image.manipulated_img
        image.manipulated_img = noisy_image

    def apply_filter(
        self, image, filter_type: str = "average", filter_size=5, **kwargs
    ):
        filtered_image = self.filter.apply_filter(
            image, filter_type, filter_size, **kwargs
        )
        image.previous_img = image.manipulated_img
        image.manipulated_img = filtered_image

    def get_edges(self, image, detector_type: str = "roberts", filter_flag=True):
        edged_image = self.edge_detector.apply_detector(
            image, detector_type, filter_flag
        )  # edged_image == [mag, direction]
